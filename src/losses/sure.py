...
def mc_div(y1, y, f, physics, tau, margin=None):
    if margin is None:
        b = torch.randn_like(y)
    else:
        # shape of the inner part of the measurements
        ip_shape = (y.size(0), y.size(1), y.size(2) - 2 * margin, y.size(3) - crop_size)
...
class SureGaussianLoss(nn.Module):
    def __init__(self, sigma, tau=1e-2, margin=None, cropped_div=False, averaged_cst=False):
